      ...
      <table class="w-full">
        ...
        <tbody>
          <tr v-for="n in 8" :key="n" class="border-t border-gray-200">
            <td class="px-6 py-2.5"><div class="h-4 w-36 bg-gray-200 rounded animate-pulse"></div></td>
          ...
          >
            ...
            <div class="px-6 py-2.5 flex gap-2">
              <router-link
                ...
                :aria-label="`Edit ${teamRows[row.index].name}`"
              ...
              <button
                ...
                :aria-label="`Delete ${teamRows[row.index].name}`"
...
const needsScroll = computed(() => teamRows.value.length * ROW_HEIGHT > window.innerHeight * 0.7)
